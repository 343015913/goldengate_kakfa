...
public class SpecificAvroMutationSerializerTest {
	...
	public void testSpecificAvroMutationSerializerWithMockRegistry() {
		...
		MutationSerializer keySerializer = new SpecificAvroMutationSerializer(schemaRegistry);
		MutationSerializer valSerializer = new SpecificAvroMutationSerializer(schemaRegistry);
		SpecificAvroMutationDeserializer keyDeserializer = new SpecificAvroMutationDeserializer(schemaRegistry);
		SpecificAvroMutationDeserializer valDeserializer = new SpecificAvroMutationDeserializer(schemaRegistry);

		keySerializer.configure(Collections.singletonMap("schema.registry.url", "http://fake-url"), true);
		valSerializer.configure(Collections.singletonMap("schema.registry.url", "http://fake-url"), false);
		keyDeserializer.configure(Collections.singletonMap("schema.registry.url", "http://fake-url"), true);
		valDeserializer.configure(Collections.singletonMap("schema.registry.url", "http://fake-url"), false);
		
		testSpecificAvroMutationSerializerImpl(keySerializer, valSerializer, keyDeserializer, valDeserializer);
	...
	public void testSpecificAvroMutationSerializerWithRealRegistry() {
		...
		SpecificAvroMutationDeserializer deserializer = new SpecificAvroMutationDeserializer();
		
		serializer.configure(Collections.singletonMap("schema.registry.url", SCHEMA_REGISTRY_URL),true );
		deserializer.configure(Collections.singletonMap("schema.registry.url", SCHEMA_REGISTRY_URL), true);

		testSpecificAvroMutationSerializerImpl(serializer, deserializer );
		
	}*/
	//Private
	private void testSpecificAvroMutationSerializerImpl(MutationSerializer keySerializer,MutationSerializer  valSerializer, SpecificAvroMutationDeserializer keyDeserializer, SpecificAvroMutationDeserializer valDeserializer ) {
		...
		Struct insertRow = new Struct(schema)
				...
				//.put("timeStamp", Table.getSQLSchemaField( schema, "timeStamp", new Timestamp(DateTime.now().getMillis()))) //TODO: Need to test Timestamp, not Date
				.put("timeStamp", Table.getSQLSchemaField( schema, "timeStamp", new Date(DateTime.now().getMillis())))
				.put("timeStamp_opt",Table.getSQLSchemaField( schema, "timeStamp_opt", null))
				.put("bytes",  Table.getSQLSchemaField( schema, "bytes", ByteBuffer.wrap("foobar".getBytes()))  );
		Struct updateRow = new Struct(schema)
		        .put("int32", Table.getSQLSchemaField( schema, "int32", 12))
				.put("float64", Table.getSQLSchemaField( schema, "float64", 12.))
				.put("string", Table.getSQLSchemaField( schema, "string", "foobar"));
		Struct deleteKey = new Struct(schema)
        .put("int32", Table.getSQLSchemaField( schema, "int32", 12));// Delete still need to have key info
		Struct pkUpdateKey = new Struct(schema)
		        .put("int32", Table.getSQLSchemaField( schema, "int32", 12));
		Struct pkUpdateRow = new Struct(schema)
        .put("int32", Table.getSQLSchemaField( schema, "int32", 13));
		...
		DeleteMutation deleteM = new DeleteMutation(table, Row.fromStruct(deleteKey));
		PkUpdateMutation pkUpdateM = new PkUpdateMutation(table, Row.fromStruct(pkUpdateKey),
      ...
      //  try { 
        ...
        res = Mutation.fromStructKeyVal(KafkaUtil.topicToDbName(topic), KafkaUtil.topicToTableName(topic), keyRes, valRes);
		...
		assertEquals("deleteM Mutation: Results should be the same as orignal",deleteM,res  );
		...
		assertEquals("pkUpdateM Mutation: Results should be the same as orignal",pkUpdateM,res  );
